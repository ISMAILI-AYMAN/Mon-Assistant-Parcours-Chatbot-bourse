...
def ingest_into_vector_store(combined_texts):
    ...
    db = Chroma(persist_directory="projet_1\data", embedding_function=OllamaEmbeddings(model="mxbai-embed-large:latest"), collection_name="rag-chroma")
...
def initialize_vector_store():
    ...
    db = Chroma(persist_directory="projet_1\insert", embedding_function=OllamaEmbeddings(model="mxbai-embed-large:latest"), collection_name="rag-chroma")
    return db
...
def main():
    ...
    if all_documents:
        ...
        db=ingest_into_vector_store(combined_texts)
        return db
    else:
        return 0
